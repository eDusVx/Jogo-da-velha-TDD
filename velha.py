...
def resultado():
    ...
    empate = 0 in board
    if empate is False and checar_vencedor() == 0:  # define se o jogo terminou empatado
        print('O jogo terminou empatado.')
        return checar_vencedor()
    if checar_vencedor() in (1, 2) and impossivel() != - 2:  # define o vencedor
        print(f'O vencedor foi o jogador {checar_vencedor()}.')
        return checar_vencedor()
    indefinido()
    return None
...
def indefinido():
    ...
    empate = 0 in board
    if empate is True and checar_vencedor() == 0:
...
board = np.array([[2, 1, 1], [2, 1, 1], [2, 2, 1]])
print(board)
print(resultado())
